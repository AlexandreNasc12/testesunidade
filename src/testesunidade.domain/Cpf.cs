namespace testesunidade.domain;

public class Cpf
{
    public const int Maxlength = 11;
    public string Numero { get; private set; }

    protected Cpf() { }

    public Cpf(string cpf)
    {
        Numero = cpf.ApenasNumeros();
    }

    public bool EstaValido()
    {
        if (string.IsNullOrWhiteSpace(Numero)) return false;

        return EstaValido(Numero);
    }

    public string ObterNumeroFormatado()
    {
        var cpfComMascara = "";

        var controle = 0;

        var controleInterno = 0;

        if (string.IsNullOrEmpty(Numero)) return Numero;

        foreach (var digito in Numero)
        {
            controle++;

            cpfComMascara += digito;

            if (controle != 3) continue;

            controleInterno++;

            if (controleInterno > 2)
                cpfComMascara += "-";
            else
                cpfComMascara += ".";

            controle = 0;
        }

        return cpfComMascara;
    }

    private static bool EstaValido(string cpf)
    {
        if (cpf.Length > 11)
            return false;

        while (cpf.Length != 11)
            cpf = '0' + cpf;

        var igual = true;
        for (var i = 1; i < 11 && igual; i++)
            if (cpf[i] != cpf[0])
                igual = false;

        if (igual || cpf == "12345678909")
            return false;

        var numeros = new int[11];

        for (var i = 0; i < 11; i++)
            numeros[i] = int.Parse(cpf[i].ToString());

        var soma = 0;
        for (var i = 0; i < 9; i++)
            soma += (10 - i) * numeros[i];

        var resultado = soma % 11;

        if (resultado == 1 || resultado == 0)
        {
            if (numeros[9] != 0)
                return false;
        }
        else if (numeros[9] != 11 - resultado)
            return false;

        soma = 0;
        for (var i = 0; i < 10; i++)
            soma += (11 - i) * numeros[i];

        resultado = soma % 11;

        if (resultado == 1 || resultado == 0)
        {
            if (numeros[10] != 0)
                return false;
        }
        else if (numeros[10] != 11 - resultado)
            return false;

        return true;
    }

    public override bool Equals(object obj)
    {
        var cpf = (Cpf)obj;

        return Numero == cpf.Numero;
    }

    public override int GetHashCode()
    {
        return HashCode.Combine(Numero);
    }

    public override string ToString()
    {
        return Numero;
    }
}
